    ...
    prepare(sql: string) {
      const bind = (...args: unknown[]) => ({
        async all<T = Record<string, unknown>>() {
          ...
          return { results: result.rows.map((r: any) => ({ ...r })) as T[] }
        },
...
async function getRole(username: string): Promise<string> {
  const row = await db.prepare('SELECT role FROM users WHERE username = ?').bind(username).first<{ role: string }>()
  if (row) return row.role
  const ownerUser = process.env.AUTH_USERNAME || 'carly'
  if (username === ownerUser) return 'owner'
  const visitorUser = process.env.VISITOR_USERNAME || 'visitor'
...
app.post('/auth/login', async (c) => {
  ...
  if (userRow) {
    ...
    const ownerUser = (process.env.AUTH_USERNAME ?? 'carly') || 'carly'
    const ownerPass = (process.env.AUTH_PASSWORD ?? 'carly') || 'carly'
    const visitorUser = (process.env.VISITOR_USERNAME ?? 'visitor') || 'visitor'
    const visitorPass = (process.env.VISITOR_PASSWORD ?? '') || ''
...
app.post('/auth/change-password', async (c) => {
  ...
  if (userRow) {
    ...
    const envPass = (process.env.AUTH_PASSWORD ?? 'carly') || 'carly'
...
async function attachMeta(db: ReturnType<typeof makeDB>, articles: Record<string, unknown>[]) {
...
app.delete('/articles/:id/tags/:tag', ownerOnly, async (c) => {
  ...
  await db.prepare('DELETE FROM article_tags WHERE article_id = ? AND tag = ?').bind(id, tag).run()
  return c.json({ success: true })
})
...
app.post('/theme/unlike', ownerOnly, async (c) => {
  const { theme } = await c.req.json()
  if (!theme) return c.json({ error: 'theme required' }, 400)
  await db.prepare('DELETE FROM theme_likes WHERE theme = ?').bind(theme).run()
  return c.json({ success: true })
})
...
async function checkAndUnlockBadges(date: string, dailyTarget: number, monthlyTarget: number): Promise<string[]> {
...
app.post('/writing/records', ownerOnly, async (c) => {
  ...
  const newBadges = await checkAndUnlockBadges(date, daily, monthly)
  return c.json({ success: true, newBadges })
})

app.get('/badges', ownerOnly, async (c) => {
  const { results } = await db.prepare('SELECT badge_id, unlock_date FROM user_badges').all()
...
app.post('/routine/preset', ownerOnly, async (c) => {
  ...
  const id = crypto.randomUUID()
  const { results: existing } = await db.prepare('SELECT MAX(sort_order) as max_order FROM routine_preset').all()
...
app.delete('/routine/preset/:id', ownerOnly, async (c) => {
  const id = c.req.param('id')
  await db.prepare('DELETE FROM routine_preset WHERE id = ?').bind(id).run()
  return c.json({ success: true })
})
...
app.get('/routine/log', async (c) => {
  ...
  const noteRow = await db.prepare('SELECT notes FROM routine_notes WHERE date = ?').bind(date).first<{ notes: string }>()
...
app.get('/routine/calendar', async (c) => {
  ...
  const prefix = `${year}-${String(month).padStart(2, '0')}`
  const totalItems = await db.prepare('SELECT COUNT(*) as cnt FROM routine_preset WHERE enabled = 1').first<{ cnt: number }>()
...
app.get('/media/platforms', async (c) => {
  const { results } = await db.prepare('SELECT * FROM media_platforms').all()
  return c.json(results)
})
...
app.get('/media/contents', async (c) => {
  ...
  const { results } = status
    ? await db.prepare('SELECT * FROM media_contents WHERE status=? ORDER BY created_at DESC').bind(status).all()
    : await db.prepare('SELECT * FROM media_contents ORDER BY created_at DESC').all()
  return c.json(results)
})
...
app.delete('/media/contents/:id', ownerOnly, async (c) => {
  await db.prepare('DELETE FROM media_contents WHERE id=?').bind(c.req.param('id')).run()
  return c.json({ success: true })
})
...
app.get('/media/collabs', async (c) => {
  const { results } = await db.prepare('SELECT * FROM media_collabs ORDER BY created_at DESC').all()
  return c.json(results)
})
...
app.delete('/media/collabs/:id', ownerOnly, async (c) => {
  await db.prepare('DELETE FROM media_collabs WHERE id=?').bind(c.req.param('id')).run()
  return c.json({ success: true })
})
