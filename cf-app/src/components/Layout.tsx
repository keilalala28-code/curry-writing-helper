...
export default function Layout({ children, onLogout, dark, onToggleTheme, role }: Props) {
  ...
  const renderNavItem = ({ path, label, icon, external }: typeof NAV_ITEMS[0], onClick?: () => void) =>
    external ? (
      <a key={path} href={path} target="_blank" rel="noopener noreferrer"
        onClick={onClick}
        className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-800 dark:hover:text-gray-200">
  ...
  const sidebarFooter = (onClick?: () => void) => (
    <div className="p-3 border-t border-gray-100 dark:border-gray-700 space-y-1">
      {!isStandalone && installPrompt && (
        <button onClick={() => { handleInstall(); onClick?.() }}
      ...
      {!isStandalone && isSafari && !installPrompt && (
        <button onClick={() => { setShowSafariTip(true); onClick?.() }}
          className="w-full text-left px-3 py-2 rounded-lg text-sm text-purple-500 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
          📲 安装到桌面
        </button>
      )}
      <button onClick={() => { onToggleTheme(); onClick?.() }}
      ...
      {isOwner && (
        <button onClick={() => { setShowChangePwd(true); onClick?.() }}
      ...
      <button onClick={() => { handleLogout(); onClick?.() }}
        className="w-full text-left px-3 py-2 rounded-lg text-sm text-gray-400 dark:text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
        退出登录
      </button>
    </div>
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      {/* ─── Desktop Sidebar (md+) ─── */}
      <aside className="hidden md:flex w-52 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex-col shadow-sm flex-shrink-0">
      ...
      <div className="flex flex-col flex-1 min-w-0 md:hidden">
        ...
        {/* Mobile Top Header */}
        <header className="sticky top-0 z-30 flex items-center justify-between px-4 h-14 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
          <button
          ...
          <h1 className="text-base font-bold text-primary">🍛 咖喱小助手</h1>
          <button
        ...
        <div
          ref={drawerRef}
          className={`fixed top-0 left-0 h-full z-50 w-64 bg-white dark:bg-gray-800 shadow-xl flex flex-col transition-transform duration-300 ease-in-out ${
      ...
      {showChangePwd && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4" onClick={() => setShowChangePwd(false)}>
