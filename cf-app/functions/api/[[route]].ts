...
请严格按以下标签格式输出，每个标签独占一行，标签后紧跟叙述内容，不输出JSON：

第一阶段铺垫事件（共4个，每个事件必须用完整叙述写满，绝不能写摘要）
事件一——男主当面偏袒女二的具体场景：
[S1E1]
事件二——女二设局陷害女主被罚的完整经过：
[S1E2]
事件三——家人（公婆/儿子/父母）当面指责女主的场景：
[S1E3]
事件四——公开场合女主被旁观者围观羞辱的场景：
[S1E4]
[TURN1]第一阶段转折点（60字内，渣男/女二做了某件让局势骤然恶化的行动，❌禁止写女主察觉任何真相）

第二阶段铺垫事件（共4个，比第一阶段更惨，最后一个事件是女主心彻底死掉的时刻）
事件一——女主身体或精神受到更深程度伤害的场景：
[S2E1]
事件二——女主向某人求助被拒绝或被反骂的场景：
[S2E2]
事件三——女主试图反抗结果被更惨地对待的场景：
[S2E3]
事件四——压垮女主情感彻底死心的最后一件事（❌不是发现真相，是被虐到极致心死了）：
[S2E4]
  ...
  try {
    const res = await fetch(`${baseUrl}/v1/messages`, {
      ...
      body: JSON.stringify({
        model,
        max_tokens: 8192,
        system: '你是一位中文网文策划专家，专门生成"被动受虐→心死离开→男主追妻"型虐文细纲。你的核心职责：每个铺垫事件标签（[S1E1]、[S1E2]、[S2E1]等）后面必须写150字以上的完整场景叙述，绝对不允许写一句话摘要。每个事件必须包含：①具体触发行为（谁做了什么，在哪里，怎么发生的）②对话原文（至少一句，用「」引号）③其他角色的连锁反应（动作+话语）④女主被迫承受的结果或处境。写完一个事件后，必须在脑中检查：有没有对话？有没有连锁反应？有没有女主的处境描写？字数够不够150字？不够必须继续补充。',
        messages: [{ role: 'user', content: prompt }],
      }),
    })
